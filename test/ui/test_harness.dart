...
Future<void> drain(WidgetTester tester) async {
  ...
  // previous pump) complete on the actual Dart event loop. Then `pump`
  // advances the fake clock and renders a frame so:
  //   - newly-yielded provider values land in the widget tree, and
  //   - any in-flight animations (dialog open/dismiss, popup menu) step
  //     forward, since `pump()` without a duration leaves the clock frozen.
  // Several passes cover chains of dependent providers and the post-write
  // notification path (ledger.changes → stream re-yield → frame).
  for (var i = 0; i < 12; i++) {
